...
func wrapText(text string, maxLen int) []string {
	if len(text) <= maxLen && !strings.Contains(text, "\n") {
		return []string{text}
	}
	...
	splitByNewline := strings.Split(text, "\n")

	for _, part := range splitByNewline {
		if part == "" {
			// Пустая строка — добавляем пустую строку
			lines = append(lines, "")
			continue
		}

		// Если часть не влазит в maxLen — разбиваем дальше
		if len(part) <= maxLen {
			lines = append(lines, part)
			continue
		}

		// Разбиваем длинную строку по пробелам
...
func GenerateLabelPdf(box BoxData, labelType string) (string, error) {
	...
	svg.WriteString("<style> text {font-family: serif;}</style>\n")
...
// GenerateCertificatePdf генерирует PDF сертификата качества из шаблона Certificate.json
func GenerateCertificatePdf(box BoxData) (string, error) {
	execPath, err := os.Executable()
	if err != nil {
		return "", err
	}
	...
	// 1. Читаем JSON-шаблон сертификата
	jsonPath := filepath.Join(appDir, "Labels", "Certificate.json")
	jsonBytes, err := os.ReadFile(jsonPath)
	if err != nil {
		return "", fmt.Errorf("файл шаблона сертификата не найден: %w", err)
	}

	var template LabelTemplate
	if err := json.Unmarshal(jsonBytes, &template); err != nil {
		return "", fmt.Errorf("ошибка парсинга JSON сертификата: %w", err)
	}
	...
	// Рисуем фиксированные текстовые поля
	for _, textField := range template.FixedTextFields {
		val := textField.Value
		if val == "" {
			val = getValueByName(textField.Name, textField.Format, box)
	...
	// Рисуем QR-коды (если есть в шаблоне)
	for _, qrTmp := range template.QRCodes {
		qrValue := fmt.Sprintf("3OS%s%s", box.Material, box.LabelCode)

		qrCode, err := qr.Encode(qrValue, qr.Q, qr.Auto)
		if err != nil {
			logger.Error("Ошибка генерации QR матрицы для сертификата: %v", err)
			continue
		}
	...
	svgDir := filepath.Join(appDir, "svg_debug")
	_ = os.MkdirAll(svgDir, 0755)
	svgPath := filepath.Join(svgDir, fmt.Sprintf("cert_%s.svg", box.LabelCode))
	_ = os.WriteFile(svgPath, svg.Bytes(), 0644)

	// 3. Рендеринг PDF
	certDir := filepath.Join(appDir, "certificates")
	_ = os.MkdirAll(certDir, 0755)
	pdfPath := filepath.Join(certDir, fmt.Sprintf("%s_cert.pdf", box.Material))

	c, err := canvas.ParseSVG(&svg)
	if err != nil {
		return "", fmt.Errorf("ошибка компиляции SVG сертификата в холст: %w", err)
	}
	...
	if err := pdfRenderer.Close(); err != nil {
		return "", fmt.Errorf("ошибка финализации PDF сертификата: %w", err)
	}

	logger.Info("Сертификат качества для материала %s сгенерирован", box.Material)
	return pdfPath, nil
}
